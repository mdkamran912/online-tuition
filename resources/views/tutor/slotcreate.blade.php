...
<meta name="csrf-token" content="{{ csrf_token() }}">
    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div id="" class="mb-3 listHeader page-title-box">
                ...
                <div style="display: flex">
                    ...
                    <select type="text" class="form-control" style="width:auto; margin-right:10px" id="selectsubject" name="selectsubject">
                        <option value="">Select Subject</option>
                        @foreach ($subjects as $subject)
                            <option value="{{$subject->id}}">{{$subject->name}}</option>
                        @endforeach
                    </select>
                    <select type="text" class="form-control" style="width:auto; margin-right:10px" id="selectstudent" name="selectstudent">
                    ...
                    <button class="btn btn-sm btn-primary bookingBtns1" type="submit"><i class="ri-calendar-todo-fill"></i> &nbsp;Search Slot</button> &nbsp;
                    <button class="btn btn-sm btn-success bookingBtns1" type="button" onclick="openclassmodal();"><i class="ri-calendar-todo-fill"></i> &nbsp;Create Slot</button>
                        </div>
                    </form>
            </div>
                    ...
                    <tbody>
                        @foreach ($slots as $slot)
                            ...
                            {{-- @endif --}}
                                ...
                                <td>
                                    @if ($slot->is_class_scheduled == 1)
                                   <a href="getclasslist"><button class="btn btn-sm btn-success bookingBtns" >Start Class</button></a></td>
                                    @elseif ($slot->is_class_scheduled == 2)
                                    
                                    @else
                                    <button onclick="newscheduleclassmodal('{{$slot->id}}','{{$slot->student_name}}','{{ \Carbon\Carbon::parse($slot->date)->format('d/m/Y') }}','{{ \Carbon\Carbon::parse($slot->slot)->format('h:i A') }}')" class="btn btn-sm btn-success bookingBtns" ><i class="ri-calendar-todo-fill"></i> &nbsp;Schedule Class</button></td>
                                    @endif
                                    @endif
                            </tr>
                        @endforeach
                    ...
                    <form action="{{ route('tutor.slots.create') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-md-12 mb-3">
                                <label>
                                    <input type="radio" id="repeatOption" name="repeatOption" value="forday" checked> Apply for one day.
                                </label>
                            </div>
                            <div class="col-12 col-md-12 mb-3">
                                <label>
                                    <input type="radio" id="repeatOption" name="repeatOption" value="forweek"> Apply for next 7 days.
                                </label>
                            </div>
                            <div class="col-12 col-md-12 mb-3">
                                <label>
                                    <input type="radio" id="repeatOption" name="repeatOption" value="formonth"> Apply for next 30 days.
                                </label>
                            </div>
        ...
        <!--Schedule modal -->
        <div class="modal fade" id="newscheduleclassmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                        ...
                        <form action="{{ route('tutor.liveclass.scheduleclass') }}" method="POST">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-md-6 col-ms-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="classstarttime" name="classstarttime" disabled>
                                    
                                </div>

                                <div class="col-12 col-md-6 col-ms-6 mb-3">
                                    <label>Class Duration(minutes)</label>
                                    <input type="tet" class="form-control" id="classduration" name="classduration" value="60" disabled>
                                </div>
                                <div class="col-12 col-md-6 col-ms-6 mb-3">
                                    <label>Class Password<span style="color:red">*</span></label>
                                    <input type="tet" class="form-control" id="classpassword" name="classpassword" required>
                                    <span class="text-danger">
                                        @error('classpassword')
                            ...
                            <div style="float:right">
                                <button type="button" class="btn btn-sm btn-danger mr-1 " onclick="newscheduleclassmodalclose();">Close</button>
